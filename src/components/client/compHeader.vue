      ...
      <li v-for="item in categories" :key="item.id" class="menu-item">
        ...
        <div class="menu-extension">
          <ul v-if="item.slug==='the-loai'" class="ul-item-1">
            <li v-for="(item,index) in genres" :key="index">{{ item.name }}</li>
          </ul>
          <ul v-else-if="item.slug==='quoc-gia'" class="ul-item-1" style="width: 450px">
            <li v-for="(item,index) in countries" :key="index">{{ item.name }}</li>
          </ul>
    ...
    <div class="repon-list-menu">
      <ul style="list-style: none; padding: 0">
        ...
        <li v-for="item in categories" :key="item.id">
          ...
          <div v-if="genreShow && item.slug==='the-loai'" class="repon-menu-extension">
            <ul style="list-style: none">
              <li v-for="(item,index) in genres" :key="index">{{ item.name }}</li>
            </ul>
          </div>
          <div v-else-if="countryShow && item.slug==='quoc-gia'" class="repon-menu-extension">
            <ul style="list-style: none">
              <li v-for="(item,index) in countries" :key="index">{{ item.name }}</li>
            </ul>
          </div>
          <div v-else-if="newShow && item.slug==='phim-moi'" class="repon-menu-extension">
            <ul style="list-style: none">
              <li v-for="year in yearList" :key="year">Phim {{ year }}</li>
            </ul>
          </div>
